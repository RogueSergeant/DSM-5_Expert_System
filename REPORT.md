...
## Abstract
Mental health services face unprecedented demand, with diagnostic complexity compounding clinical workload pressures. This project presents a hybrid diagnostic decision support system that integrates symbolic reasoning with large language model (LLM) capabilities to assist clinicians in applying DSM-5 diagnostic criteria systematically.
The system employs a three-tier architecture: Tier A uses Prolog-based inference for objective criteria such as symptom counts, duration thresholds, and onset constraints; Tier B leverages LLM assessment with clinician oversight for subjective judgements including clinical significance and severity; Tier C integrates both reasoning pathways to produce ranked diagnoses with full explanation chains. An A* search algorithm optimises the diagnostic pathway, reducing unnecessary questioning through intelligent branch pruning.
The knowledge base covers five representative disorders (Major Depressive Disorder, Generalised Anxiety Disorder, ADHD, PTSD, and Autism Spectrum Disorder), with validation against 30-50 synthetic clinical vignettes. Evaluation targets include greater than 90% accuracy for unambiguous cases and over 40% reduction in required clinical queries compared to exhaustive assessment. This proof-of-concept demonstrates the viability of combining deterministic rule-based reasoning with flexible natural language interpretation for clinical decision support.
---
